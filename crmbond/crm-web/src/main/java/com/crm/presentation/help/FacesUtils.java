...
public final class FacesUtils {
	...
	public static String getMessage(String key) {
		return ResourceBundle.getBundle(MESSAGE_FILE_NAME, FacesContext.getCurrentInstance().getViewRoot().getLocale()).getString(key);
	}
	...
	private static void addFacesMessage(Severity severity, String messageKey, ResourceBundle bundle, String... params) {
		...
		try {
			if (params.length == 0) {
				...
				message = bundle.getString(messageKey);
				MessageFormat mf = new MessageFormat(message,FacesContext.getCurrentInstance().getViewRoot().getLocale());
